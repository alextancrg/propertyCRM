...
export function AiSettings({
  ...
  async function runReminders() {
    ...
    try {
      ...
      if (res.ok) {
        setRunResult(`Sent ${data.reminders ?? 0} reminder(s) · escalated ${data.escalated ?? 0} · skipped ${data.skipped ?? 0}.`);
          ...
          <div className="flex items-start gap-4">
            ...
            <div>
              ...
              <p className="max-w-xl text-sm text-slate-500">
                Automates tenant conversations — rent reminders, maintenance triage, and viewing scheduling — over WhatsApp.
              </p>
              <div className="mt-2 flex items-center gap-2">
                ...
                {config.enabled && <span className="text-xs text-slate-400">Automations are active</span>}
              </div>
            </div>
          </div>
        ...
        <form onSubmit={saveForm} className="card space-y-5 p-6">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <select value={config.provider} onChange={(e) => setConfig({ ...config, provider: e.target.value })} className="input cursor-pointer">
                <option value="mock">Built-in assistant</option>
                <option value="openai">OpenAI (API key)</option>
              </select>
            </div>
            <div>
              <label className="label mb-1">Model</label>
              <input value={config.model} onChange={(e) => setConfig({ ...config, model: e.target.value })} className="input" placeholder="gpt-4o-mini" />
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              <label className="label mb-1">Authorized tenants</label>
              <input value={config.tenantNames} onChange={(e) => setConfig({ ...config, tenantNames: e.target.value })} className="input" placeholder="Comma-separated names" />
            </div>
          </div>
          ...
          <div className="rounded-xl bg-slate-50 p-4 text-xs leading-relaxed text-slate-500">
            <p className="mb-1 font-bold uppercase tracking-wide text-slate-400">WhatsApp webhook</p>
            <p>
              Point Meta Cloud API to <code className="rounded bg-slate-200 px-1">/api/whatsapp/webhook</code> and set
              the <code className="rounded bg-slate-200 px-1">WHATSAPP_VERIFY_TOKEN</code> environment variable.
              When the agent is disabled, inbound messages are recorded but not answered.
            </p>
          </div>
        </form>
